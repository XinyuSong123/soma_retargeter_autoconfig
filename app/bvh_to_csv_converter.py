...
class Viewer:
    def __init__(self, viewer, config):
        ...
        self.retarget_target_options = pipeline_utils.get_supported_target_names()
        self.retarget_solver_options = ['Newton']
        self.retarget_solver_idx     = 0
        configured_target = robot_registry_parser.resolve_robot_name(self.config.get('retarget_target', 'unitree_g1'))
        if configured_target not in self.retarget_target_options:
            self.retarget_target_options.append(configured_target)
        self.config['retarget_target'] = configured_target
        self.retarget_target_idx     = self.retarget_target_options.index(configured_target)
        self.retarget_source_idx     = 0
        self.retarget_default_config = {}
        self.ui_ik_iterations = 0
        self.ui_joint_limit_weight = 0.0
        self.ui_smooth_joint_filter_weight = 0.0
        self.ui_num_initialization_frames = 0
        self.ui_num_stabilization_frames = 0
        self.ui_enable_post_processing = False
        ...
        self._set_target_robot(self.retarget_target_options[self.retarget_target_idx], reset_csv_buffers=False)
    ...
    def load_csv_file(self, path):
        ...
        self.robot_csv_animation_buffers[0] = csv_utils.load_csv_auto(path, retarget_target)
    ...
    def _rebuild_robot_model(self, retarget_target):
    ...
    def _reset_retarget_settings(self, retarget_source, retarget_target):
        ...
        self.retarget_default_config = copy.deepcopy(
            pipeline_utils.get_retargeter_config(source_type, target_type)
        )

        self.ui_ik_iterations = int(self.retarget_default_config.get('ik_iterations', 24))
        self.ui_joint_limit_weight = float(self.retarget_default_config.get('joint_limit_weight', 10.0))
        self.ui_smooth_joint_filter_weight = float(
            self.retarget_default_config.get('smooth_joint_filter_weight', 5.5)
        )
        self.ui_num_initialization_frames = int(
            self.retarget_default_config.get('num_initialization_frames', 0)
        )
        self.ui_num_stabilization_frames = int(
            self.retarget_default_config.get('num_stabilization_frames', 0)
        )
        self.ui_enable_post_processing = bool(
            self.retarget_default_config.get('enable_post_processing', True)
        )

    def _build_runtime_retarget_config(self):
        runtime_config = copy.deepcopy(self.retarget_default_config)
        runtime_config['ik_iterations'] = max(1, int(self.ui_ik_iterations))
        runtime_config['joint_limit_weight'] = max(0.0, float(self.ui_joint_limit_weight))
        runtime_config['smooth_joint_filter_weight'] = max(
            0.0, float(self.ui_smooth_joint_filter_weight)
        )
        runtime_config['num_initialization_frames'] = max(
            0, int(self.ui_num_initialization_frames)
        )
        runtime_config['num_stabilization_frames'] = max(
            0, int(self.ui_num_stabilization_frames)
        )
        runtime_config['enable_post_processing'] = bool(self.ui_enable_post_processing)
        return runtime_config

    def _set_target_robot(self, retarget_target, reset_csv_buffers=True):
        retarget_target = robot_registry_parser.resolve_robot_name(retarget_target)
        if retarget_target not in self.retarget_target_options:
            self.retarget_target_options.append(retarget_target)
        self.config['retarget_target'] = retarget_target
        self.retarget_target_idx = self.retarget_target_options.index(retarget_target)
        self._rebuild_robot_model(retarget_target)
        self._reset_retarget_settings(
            self.retarget_source_options[self.retarget_source_idx],
            retarget_target,
        )

        if reset_csv_buffers:
            self.robot_csv_animation_buffers = [None for _ in range(self.num_robots)]
            self.compute_playback_total_time()
    ...
    def retarget_motion(self):
        ...
        if (retarget_solver == 'Newton'):
            ...
            pipeline = newton_pipeline.NewtonPipeline(
                ...
                retarget_source,
                retarget_target,
                retarget_config=self._build_runtime_retarget_config(),
            )
        else:
    ...
    def ui_scene_options(self, ui):
        ...
        if ui.collapsing_header("Motion", flags=ui.TreeNodeFlags_.default_open):
            ...
            ui.text("Target Robot:")
            ui.same_line()
            ui.set_next_item_width(160)
            changed, new_target_idx = ui.combo(
                "##TargetRobot",
                self.retarget_target_idx,
                self.retarget_target_options
            )
            if changed:
                self._set_target_robot(self.retarget_target_options[new_target_idx])

            ui.align_text_to_frame_padding()
            ui.text(f"Current Model: {self.retarget_target_options[self.retarget_target_idx]}")
            ...
            ui.spacing()
            if ui.collapsing_header("Retarget Settings", flags=ui.TreeNodeFlags_.default_open):
                ui.separator()
                ui.text("Applies to the next Retarget run.")

                changed, new_ik_iterations = ui.input_int("IK Iterations", int(self.ui_ik_iterations))
                if changed:
                    self.ui_ik_iterations = max(1, new_ik_iterations)

                changed, new_joint_limit_weight = ui.input_float(
                    "Joint Limit Weight",
                    float(self.ui_joint_limit_weight),
                    format="%.3f",
                )
                if changed:
                    self.ui_joint_limit_weight = max(0.0, new_joint_limit_weight)

                changed, new_smooth_joint_filter_weight = ui.input_float(
                    "Smooth Joint Weight",
                    float(self.ui_smooth_joint_filter_weight),
                    format="%.3f",
                )
                if changed:
                    self.ui_smooth_joint_filter_weight = max(0.0, new_smooth_joint_filter_weight)

                changed, new_initialization_frames = ui.input_int(
                    "Init Frames",
                    int(self.ui_num_initialization_frames),
                )
                if changed:
                    self.ui_num_initialization_frames = max(0, new_initialization_frames)

                changed, new_stabilization_frames = ui.input_int(
                    "Stabilization Frames",
                    int(self.ui_num_stabilization_frames),
                )
                if changed:
                    self.ui_num_stabilization_frames = max(0, new_stabilization_frames)

                _, self.ui_enable_post_processing = ui.checkbox(
                    "Enable Post Processing",
                    self.ui_enable_post_processing,
                )

                if ui.button("Reset Retarget Defaults"):
                    self._reset_retarget_settings(
                        self.retarget_source_options[self.retarget_source_idx],
...
def main():
    ...
    if args.config is None:
        ...
        if config_path is None:
            config_path = robot_registry_parser.get_generated_converter_config_path(robot_hint)
        args.config = str(config_path) if config_path is not None else None
    if args.config is None or not pathlib.Path(args.config).exists():
        print(f"[ERROR]: Main config json file not found: {args.config}")
        exit(1)

    config = io_utils.load_json(args.config)
    if args.robot:
        config['retarget_target'] = robot_registry_parser.resolve_robot_name(args.robot)
    else:
        config['retarget_target'] = robot_registry_parser.resolve_robot_name(
