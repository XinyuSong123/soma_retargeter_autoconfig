...
def test_step3_4_runtime_code_has_no_model_keyed_residual_tuning_tables() -> None:
...
def _implementation_files() -> list[Path]:
    return [
        ...
        Path("soma_retargeter/runtime/v3/quality_metrics.py"),
        ...
        Path("scripts/audit_retargeting_v3_step3_4_global_residual_quality.py"),
    ]
...
def _suspicious_table_name(name: str) -> bool:
    lowered = name.lower()
    has_gate_term = any(term in lowered for term in ("threshold", "whitelist", "allowlist", "gate", "weight", "damping", "residual"))
