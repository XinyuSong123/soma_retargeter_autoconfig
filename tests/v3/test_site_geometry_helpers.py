...
""".strip()
    ...
    assert site.source == "compiled_geom_bounds"
...
def test_rpo_body_geometry_bounds_prefers_compiled_mesh_vertices():
    adapter = MuJoCoRuntimeModelAdapter("assets/robots/atom01/mjcf/atom01.xml")

    bounds = body_geometry_bounds(adapter, "left_ankle_roll_link")

    assert "compiled_mesh_vertices" in bounds.evidence[0]
    np.testing.assert_allclose(bounds.minimum, [-0.05400000428651622, -0.029000000877762937, -0.04500000282002089])
    np.testing.assert_allclose(bounds.maximum, [0.10400000044240255, 0.04785000340505435, 0.010500001132944935])
