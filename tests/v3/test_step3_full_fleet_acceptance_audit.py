...
def test_full_fleet_audit_accepts_complete_44_row_evidence(tmp_path: Path) -> None:
    ...
    assert len(REQUIRED_ARTIFACT_FILES) == 9
...
def test_audit_blocks_missing_pipeline_controls(tmp_path: Path) -> None:
    ...
    assert result.status == "BLOCKED"
    assert result.gate_counts["pipeline_controls_present"] >= 3
...
def _write_passing_evidence(tmp_path: Path) -> tuple[Path, Path]:
    ...
    _write_json(artifact_dir / "environment.json", {"schema_version": 1, "git_status_short": ""})
    ...
    _write_json(artifact_dir / "full_fleet_matrix.json", {"schema_version": 1, "matrix": _matrix_rows()})
    _write_json(
...
def _matrix_rows() -> list[dict]:
    ...
    for model_id, robot_type, status, expected_capability in model_specs:
        ...
        if status == "negative_control_passed":
            ...
            row.update(
                {
                    "runtime_quality_status": "partial_structural_caveat",
                    "quality_classification": "partial_structural_caveat",
            ...
            row.update(
                {
                    "runtime_quality_status": "quality_passed",
                    "quality_classification": "full_fleet_quality_pass",
                    "quality_evaluated": True,
                }
            )
        rows.append(row)
