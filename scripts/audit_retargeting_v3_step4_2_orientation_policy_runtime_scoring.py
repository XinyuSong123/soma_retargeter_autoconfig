#!/usr/bin/env python3
"""Audit Step 4.2 orientation policy runtime-scoring artifacts."""
...
DEFAULT_ARTIFACT_DIR = Path("artifacts/retargeting_v3_step4_2_orientation_policy_runtime_scoring")
DEFAULT_BASELINE_ARTIFACT_DIR = Path("artifacts/retargeting_v3_step4_1_orientation_residual_breakthrough")
...
SCAN_PATHS = (
    ...
    "soma_retargeter/tools/step4_2_orientation_policy_runtime_scoring.py",
...
def run_audit(
    ...
    *,
    ...
    runtime_delta = _read_json(artifact_dir / "runtime_scoring_delta_vs_step4_1.json")
    quality_delta = _read_json(artifact_dir / "quality_delta_vs_step4_1.json")
    impact = _read_json(artifact_dir / "orientation_policy_runtime_impact_report.json")
    policy_matrix = _read_json(artifact_dir / "orientation_policy_runtime_scoring_matrix.json")
    active_vs_diagnostic = _read_json(artifact_dir / "active_vs_diagnostic_policy_matrix.json")
    gate_report = _read_json(artifact_dir / "gate_reconciliation_report.json")
    normalization = _read_json(artifact_dir / "scoring_normalization_audit.json")
    orientation_matrix = _read_json(artifact_dir / "orientation_integrated_residual_matrix.json")
    clip_consistency = _read_json(artifact_dir / "orientation_integrated_clip_consistency_matrix.json")
    ...
    findings.extend(_audit_baseline_step4_1(baseline_artifact_dir, baseline_summary, summary, ledger, runtime_delta))
    ...
    findings.extend(_audit_policy_artifacts(policy_matrix, active_vs_diagnostic, orientation_matrix, clip_consistency, summary))
    findings.extend(_audit_gate_reconciliation(gate_report, summary, runtime_delta))
    findings.extend(_audit_runtime_delta(runtime_delta, quality_delta, baseline_summary, summary))
    findings.extend(_audit_release_candidate_status(summary, ledger, runtime_delta, impact, gate_report))
    findings.extend(_audit_normalization(normalization, runtime_delta))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--baseline-step4-1-artifact-dir", "--baseline-artifact-dir", dest="baseline_artifact_dir", default=str(DEFAULT_BASELINE_ARTIFACT_DIR))
...
def _audit_required_artifacts(artifact_dir: Path) -> list[Finding]:
    ...
    if not artifact_dir.exists():
        return [_finding("missing_required_artifacts", _display_path(artifact_dir), "Step 4.2 artifact directory is missing", {})]
    for relative in REQUIRED_ARTIFACT_FILES:
        if not (artifact_dir / relative).exists():
            findings.append(_finding("missing_required_artifacts", relative, "required Step 4.2 artifact is missing", {}))
...
def _audit_baseline_step4_1(
    ...
    if not baseline_artifact_dir.exists():
        findings.append(_finding("baseline_step4_1", _display_path(baseline_artifact_dir), "Step 4.1 baseline artifact dir is missing", {}))
    expected = {
        "in_scope_total": 44,
    ...
    for field, expected_value in expected.items():
        if _as_int(baseline_summary.get(field)) != expected_value:
            findings.append(_finding("baseline_step4_1", field, "baseline Step 4.1 summary mismatch", {"actual": baseline_summary.get(field), "expected": expected_value}))
    if baseline_summary.get("orientation_selected_policy") != "parent_relative_runtime_inv_target":
        findings.append(_finding("baseline_step4_1", "orientation_selected_policy", "Step 4.1 selected policy must be parent-relative", {"actual": baseline_summary.get("orientation_selected_policy")}))
    base_head = summary.get("base_step4_1_final_head") or ledger.get("base_step4_1_final_head")
    if not _is_concrete_sha(base_head):
        findings.append(_finding("baseline_step4_1", "base_step4_1_final_head", "base Step 4.1 final HEAD must be concrete", {"value": base_head}))
    if runtime_delta.get("baseline_counts") and _as_int(runtime_delta["baseline_counts"].get("runtime_quality_passed_count")) != 0:
        findings.append(_finding("baseline_step4_1", "runtime_scoring_delta_vs_step4_1.json", "baseline counts must preserve Step 4.1 truth", {}))
    return findings
...
def _audit_clean_provenance(
    ...
    for subject, payload in (("quality_summary.json", summary), ("acceptance_ledger.json", ledger), ("runtime_scoring_delta_vs_step4_1.json", runtime_delta)):
...
def _audit_solver_and_status_counts(
    ...
    for field in ("solver_backed_smoke_attempted_count", "solver_backed_completed_count", "solver_backed_count"):
        if expected_counts[field] < 32 or _as_int(summary.get(field)) != expected_counts[field]:
            findings.append(_finding("solver_backed_counts", field, "Step 4.2 must preserve solver-backed coverage", {"rows": expected_counts[field], "summary": summary.get(field)}))
    for field, expected in (("residual_only_count", 0), ("runtime_quality_failed_count", 0)):
        if expected_counts[field] != expected or _as_int(summary.get(field)) != expected:
            findings.append(_finding("solver_backed_counts", field, "Step 4.2 invariant mismatch", {"rows": expected_counts[field], "summary": summary.get(field), "expected": expected}))
    completed_models = {
...
def _audit_policy_artifacts(
    ...
    findings = []
    for name, payload in (
        ("orientation_policy_runtime_scoring_matrix.json", policy_matrix),
        ("active_vs_diagnostic_policy_matrix.json", active_vs_diagnostic),
        ("orientation_integrated_residual_matrix.json", orientation_matrix),
        ("orientation_integrated_clip_consistency_matrix.json", clip_consistency),
    ):
        rows = _matrix_rows(payload)
        if not rows:
            findings.append(_finding("orientation_policy_runtime_scoring", name, "policy/runtime scoring matrix is empty", {}))
    if _as_int(orientation_matrix.get("row_count")) != 32 or len(_matrix_rows(orientation_matrix)) != 32:
        findings.append(_finding("orientation_policy_runtime_scoring", "orientation_integrated_residual_matrix.json", "expected 32 full-humanoid orientation rows", {"row_count": orientation_matrix.get("row_count")}))
    if summary.get("orientation_policy_active_for_scoring") is not True:
        findings.append(_finding("orientation_policy_runtime_scoring", "quality_summary.json", "Step 4.2 policy must be active for scoring", {}))
    for payload_name, payload in (("quality_summary.json", summary), ("orientation_policy_runtime_scoring_matrix.json", policy_matrix)):
        if payload.get("orientation_policy_production_default_changed") is True or payload.get("production_default_changed") is True:
            findings.append(_finding("orientation_policy_runtime_scoring", payload_name, "production default must remain unchanged", {}))
        if payload.get("runtime_override_default_enabled") is True:
            findings.append(_finding("orientation_policy_runtime_scoring", payload_name, "runtime override must not default-enable", {}))
    for index, row in enumerate(_matrix_rows(policy_matrix)[:32]):
        if row.get("active_runtime_scoring_policy") != "parent_relative_runtime_inv_target":
            findings.append(_finding("orientation_policy_runtime_scoring", f"row[{index}]", "active scoring policy must be selected parent-relative policy", {"row": row}))
        if row.get("production_default_changed") is True or row.get("runtime_override_default_enabled") is True:
            findings.append(_finding("orientation_policy_runtime_scoring", f"row[{index}]", "policy row changed production defaults", {"row": row}))
    return findings


def _audit_gate_reconciliation(gate_report: dict[str, Any], summary: dict[str, Any], runtime_delta: dict[str, Any]) -> list[Finding]:
    findings = []
    required = (
    ...
    for field in required:
        if field not in gate_report:
            findings.append(_finding("gate_reconciliation", "gate_reconciliation_report.json", "required gate field missing", {"field": field}))
    if gate_report.get("pass_gate_thresholds_unchanged") is not True or gate_report.get("warn_gate_thresholds_unchanged") is not True:
        findings.append(_finding("gate_reconciliation", "gate_reconciliation_report.json", "runtime quality gates must remain unchanged", {}))
    if gate_report.get("gates_weakened") is True:
        findings.append(_finding("gate_reconciliation", "gate_reconciliation_report.json", "runtime quality gates were weakened", {}))
    if _as_int(summary.get("runtime_quality_passed_count")) == 0 and not gate_report.get("why_no_pass_if_zero"):
        findings.append(_finding("gate_reconciliation", "gate_reconciliation_report.json", "zero pass rows require explicit blockers", {}))
    if runtime_delta.get("gate_blocker_taxonomy") and gate_report.get("per_gate_blocker_counts") == {}:
        findings.append(_finding("gate_reconciliation", "gate_reconciliation_report.json", "gate blocker taxonomy is missing", {}))
    return findings
...
def _audit_runtime_delta(
    ...
    if not runtime_delta:
        return [_finding("runtime_scoring_delta_vs_step4_1", "runtime_scoring_delta_vs_step4_1.json", "runtime scoring delta is missing", {})]
    for field in ("in_scope_total", "full_humanoid_total", "partial_total", "negative_total", "solver_backed_count", "residual_only_count", "runtime_quality_failed_count"):
        ...
        if field in baseline_counts and _as_int(baseline_counts.get(field)) != _as_int(baseline_summary.get(field)):
            findings.append(_finding("runtime_scoring_delta_vs_step4_1", field, "baseline count mismatch", {"delta": baseline_counts.get(field), "baseline": baseline_summary.get(field)}))
        if field in current_counts and _as_int(current_counts.get(field)) != _as_int(summary.get(field)):
            findings.append(_finding("runtime_scoring_delta_vs_step4_1", field, "current count mismatch", {"delta": current_counts.get(field), "summary": summary.get(field)}))
    for field in (
        ...
        if field not in runtime_delta:
            findings.append(_finding("runtime_scoring_delta_vs_step4_1", "runtime_scoring_delta_vs_step4_1.json", "required delta field missing", {"field": field}))
    if runtime_delta.get("regressions"):
        findings.append(_finding("runtime_scoring_delta_vs_step4_1", "regressions", "runtime scoring delta records pipeline regressions", {"regressions": runtime_delta.get("regressions")}))
    if quality_delta and quality_delta.get("runtime_scoring_delta_vs_step4_1") in ({}, None):
        findings.append(_finding("quality_delta_vs_step4_1", "quality_delta_vs_step4_1.json", "quality delta must reference runtime scoring delta", {}))
    return findings
...
def _audit_release_candidate_status(
    ...
    if status not in VALID_RELEASE_STATUSES:
        findings.append(_finding("release_candidate_status", "quality_summary.json", "invalid Step 4.2 release status", {"status": status}))
    ...
    target = _active_scoring_breakthrough(summary, runtime_delta, gate_report)
    if status == "PASS_RC":
        requirements = {
            ...
            "active_scoring_breakthrough": target,
            "policy_active": summary.get("orientation_policy_active_for_scoring") is True,
            "production_default_unchanged": summary.get("orientation_policy_production_default_changed") is False,
            "runtime_override_default_disabled": summary.get("runtime_override_default_enabled") is False,
            "runtime_quality_failed_count_zero": _as_int(summary.get("runtime_quality_failed_count")) == 0,
            "no_delta_regressions": not runtime_delta.get("regressions"),
        ...
        if _blocked_diagnostics_complete(runtime_delta, impact, gate_report) is not True:
            findings.append(_finding("release_candidate_status", "quality_summary.json", "blocked scoring/gate status requires complete diagnostics", {"status": status}))
    elif status.startswith("BLOCKED") and target and not runtime_delta.get("regressions"):
        findings.append(_finding("release_candidate_status", "quality_summary.json", "blocked status contradicts active scoring breakthrough evidence", {"status": status}))
    return findings


def _audit_normalization(normalization: dict[str, Any], runtime_delta: dict[str, Any]) -> list[Finding]:
    findings = []
    if normalization.get("raw_residual_always_retained") is not True:
        findings.append(_finding("normalization_integrity", "scoring_normalization_audit.json", "raw residual must be retained", {}))
    if normalization.get("normalization_hides_raw_residual_regression") is True or runtime_delta.get("normalization_hides_raw_residual_regression") is True:
        findings.append(_finding("normalization_integrity", "scoring_normalization_audit.json", "normalization hides raw residual regression", {"normalization": normalization, "delta": runtime_delta}))
    if normalization.get("denominator_inflation_detected") is True:
        findings.append(_finding("normalization_integrity", "scoring_normalization_audit.json", "denominator inflation detected", {"suspicious_rows": normalization.get("suspicious_rows")}))
    if normalization.get("normalization_robot_specific") is True:
        findings.append(_finding("normalization_integrity", "scoring_normalization_audit.json", "normalization must not be robot-specific", {}))
    return findings
...
def _audit_configs(solver_config: dict[str, Any], pipeline_config: dict[str, Any]) -> list[Finding]:
    ...
    policy = solver_config.get("global_orientation_residual_policy") if isinstance(solver_config.get("global_orientation_residual_policy"), dict) else {}
    config = pipeline_config.get("config") if isinstance(pipeline_config.get("config"), dict) else {}
    if policy.get("active_for_runtime_scoring") is not True or policy.get("active_runtime_scoring_policy") != "parent_relative_runtime_inv_target":
        findings.append(_finding("global_config", "solver_config.json", "active scoring policy must be recorded in solver config", {"policy": policy}))
    if config.get("enable_parent_relative_orientation_runtime_scoring") is not True:
        findings.append(_finding("global_config", "pipeline_config.json", "explicit Step 4.2 scoring flag must be recorded", {"config": config}))
    if policy.get("production_default_changed") is True or config.get("orientation_policy_production_default_changed") is True:
...
def _audit_final_head_ci(committed_record: dict[str, Any], record: dict[str, Any], source_root: Path) -> list[Finding]:
    ...
    for job in REQUIRED_FINAL_HEAD_CI_JOBS:
        if job_conclusions.get(job) != "success":
            findings.append(_finding("final_head_ci", job, "required Step 4.2 CI job is not successful", {"job_conclusions": job_conclusions, "committed_record": committed_record}))
    return findings


def _active_scoring_breakthrough(summary: dict[str, Any], runtime_delta: dict[str, Any], gate_report: dict[str, Any]) -> bool:
    return bool(
        ...
        or (
            ...
            and gate_report.get("gates_weakened") is not True
        )
        or float(runtime_delta.get("p95_normalized_task_residual_p95_delta", 0.0) or 0.0) <= -0.05
    )


def _blocked_diagnostics_complete(runtime_delta: dict[str, Any], impact: dict[str, Any], gate_report: dict[str, Any]) -> bool:
    return bool(runtime_delta and impact and gate_report and gate_report.get("why_no_pass_if_zero") is not None)
...
def _github_json(url: str) -> dict[str, Any]:
    request = urllib.request.Request(url, headers={"Accept": "application/vnd.github+json", "User-Agent": "codex-step4-2-audit"})
    try:
