...
- Self-collision proxy and high-risk pair metadata are compile-time only; runtime collision barrier objectives are not implemented.
...
- Ground/root-height reporting is not yet part of the profile/report schema beyond optional runtime `ground_barrier` configuration.
