...
Latest verified result: `84 passed`. The benchmark CLI tests are in `tests/test_benchmark_retargeting.py`.
...
Current bounded gate status is `failed`: RoboParty RPO and generic Unitree G1 pass penetration, root tilt, torso leakage, and actuated smoothness gates on the selected high-motion windows, and both record zero priority-guard rollbacks while protecting hard joint-limit penetration. The hard-penetration guard uses the CUDA graph-capture fast path, and the compiled-profile IK default uses 8 solver iterations instead of 24 to bound overfitting and runtime. The range-normalized joint motion limiter brings RPO actuated `velocity_p95/acceleration_p95` to `24.19/754.18` versus legacy `28.91/1626.89`, and Unitree G1 to `35.43/924.11` versus legacy `56.51/6388.52`. After target-frame alignment, RPO hand/foot RMSE is `0.60/0.80` versus legacy `0.22/0.18`, and Unitree G1 hand/foot RMSE is `0.66/0.78` versus legacy `0.55/0.45`. Both robots still fail hand/foot tracking and runtime-overhead gates, so the next acceptance work is tracking quality and remaining runtime reduction rather than static-output rollback.
