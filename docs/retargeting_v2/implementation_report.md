...
Current bounded gate status is `failed`: RoboParty RPO and generic Unitree G1 pass penetration, torso leakage, and actuated smoothness gates on the selected high-motion windows, and both record zero priority-guard rollbacks while protecting hard joint-limit penetration. The hard-penetration guard uses the CUDA graph-capture fast path. Compiled-profile runtime uses morphology-scheduled IK iteration defaults: single-axis torso profiles use 4 iterations, while multi-axis torso profiles keep 8 iterations to preserve Unitree tracking gates. The range-normalized joint motion limiter brings RPO actuated `velocity_p95/acceleration_p95` to `21.90/554.54` versus legacy `28.91/1626.89`, and Unitree G1 to `36.93/839.40` versus legacy `56.51/6388.52`. With reachable semantic-site offsets and morphology-scheduled weights applied to position IK, Unitree G1 hand/foot RMSE passes at `0.55/0.43` versus legacy `0.55/0.45`, and root tilt now passes at `0.93` versus legacy `1.60`. Weak single-axis torso tracking plus the 4-iteration single-axis runtime default keeps RPO hand/foot RMSE at `0.40/0.43` versus legacy `0.22/0.18`, and keeps RPO root tilt below legacy at `1.57` versus `1.67`, but RPO hand/foot tracking still fails the report-only gates. RPO hand residuals remain relatively balanced (`axis_rmse [0.21, 0.21, 0.24]`, `mean_error [-0.10, -0.06, -0.03]`), while RPO foot residuals remain elevated, especially y/z (`axis_rmse [0.18, 0.26, 0.25]`). Unitree G1 now fails only runtime, while RPO still fails hand/foot tracking and runtime. Runtime profiling shows the remaining overhead is solver dominated rather than BVH load or metric evaluation: RPO v2 improves to about `40.57` output frames/s versus legacy `192.74`, and Unitree G1 v2 remains about `20.18` output frames/s versus legacy `175.33`. The next acceptance work is RPO reach-limited hand/foot tracking and solver-kernel/runtime reduction, especially removing direction/pole objectives from the autodiff Jacobian slow path.
