...
Latest result before this report update: `75 passed`. The benchmark CLI tests are in `tests/test_benchmark_retargeting.py`.
...
Current artifact scope includes compile-level validation plus bounded runtime rollouts for the first two resolved BVH motions. The benchmark now runs legacy and v2 compare modes through separate runtime configs: legacy uses the raw string `ik_map` with default position/rotation weights and no compiled profile, while v2 uses the compiled morphology-aware profile, direction/pole tasks, segment-local scaler, and priority guard. Runtime metrics include task residual by type/priority when profile tasks are evaluable, torso reachable/unreachable residuals for projected torso tasks, joint-limit margin, foot slide or structured unavailability, penetration, root tilt, hand/foot position RMSE, velocity/acceleration p95, solver iterations, and fallback counts. `benchmark_summary.json` records `resolved_motions`, and `benchmark_frames.csv` includes the per-motion BVH path for each motion-level metric row. `benchmark_gates.json` records report-only legacy/v2 acceptance gates by default; `--strict-gates` returns exit code 4 on gate failure. `registry_coverage.json` records coverage for RPO, generic Unitree G1, G1 23DoF, G1 29DoF, E3 v2, and OLI.
...
Unitree G1 currently produces a v2 artifact and separated legacy/v2 bounded runtime metrics. The bounded sample shows legacy foot penetration while v2 reports zero measured penetration, but profile-task residuals are structurally unavailable because its chain summary still shows zero reachable ranks and minimal chain lengths; this is a known limitation of the current registered model/config path and needs deeper registry/morphology work before runtime acceptance.

Registry coverage currently reports RoboParty RPO as `ready`, generic `unitree_g1` as `registered_incomplete` because MJCF morphology is missing, and `unitree_g1_23dof`, `unitree_g1_29dof`, `e3_v2`, and `oli` as `missing_registration`.

Current bounded gate status is `failed`: RoboParty RPO fails hand RMSE, foot RMSE, and runtime-overhead gates; generic Unitree G1 fails hand RMSE and runtime-overhead gates, with torso-unreachable residual unavailable due incomplete morphology.
