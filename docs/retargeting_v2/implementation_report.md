...
Latest verified result: `94 passed, 6 subtests passed`. The benchmark CLI tests are in `tests/test_benchmark_retargeting.py`.
...
Current bounded gate status is `failed`: RoboParty RPO, generic Unitree G1, and synthetic E3 v2 pass penetration, torso leakage, and actuated smoothness gates on the selected high-motion windows, and each records zero priority-guard rollbacks while protecting hard joint-limit penetration. The hard-penetration guard uses the CUDA graph-capture fast path. Contact heuristic inference now accepts both Warp transforms and numpy transform rows, so the synthetic E3 runtime no longer disables contact locks due to transform type mismatch; its legacy and v2 contact configs also receive virtual sole anchor offsets. Compiled-profile runtime uses morphology-scheduled IK iteration defaults: single-axis torso profiles use 4 iterations, while multi-axis torso profiles keep 8 iterations to preserve Unitree tracking gates. Direction objectives now use an analytic Jacobian only for multi-axis torso profiles; single-axis torso profiles keep the autodiff direction objective because the analytic path changed bounded RPO tracking. Pole-vector objectives have an experimental analytic Jacobian implementation with expanded finite-difference validation, but compiled runtime configs keep it disabled until benchmark-safe activation because direct enablement regressed bounded hand/foot/root tracking. The range-normalized joint motion limiter brings RPO actuated `velocity_p95/acceleration_p95` to `21.87/578.49` versus legacy `28.91/1626.89`, Unitree G1 to `36.89/863.50` versus legacy `56.51/6388.52`, and synthetic E3 v2 to `8.27/214.00` versus legacy `32.88/3464.22`. With reachable semantic-site offsets and morphology-scheduled weights applied to position IK, Unitree G1 hand/foot RMSE passes at `0.57/0.44` versus legacy `0.55/0.45`, and root tilt now passes at `0.89` versus legacy `1.60`. Synthetic E3 v2 passes foot/root/torso gates (`0.31` foot RMSE versus legacy `0.42`, root tilt `0.87` versus `1.14`) after low-rank no-wrist hand tasks are softened across torso profiles, but fails hand tracking (`1.20` versus legacy `0.59`) and runtime. Weak single-axis torso tracking, the 4-iteration single-axis runtime default, and softened low-rank endpoint weights keep RPO hand/foot RMSE at `0.39/0.41` versus legacy `0.22/0.18`, and keep RPO root tilt slightly below legacy at `1.669` versus `1.673`, but RPO hand/foot tracking still fails the report-only gates. RPO hand residuals remain relatively balanced (`axis_rmse [0.21, 0.21, 0.23]`, `mean_error [-0.10, -0.06, -0.03]`), while RPO foot residuals remain elevated (`axis_rmse [0.18, 0.24, 0.22]`). Unitree G1 now fails only runtime, while RPO fails hand/foot tracking and runtime, and synthetic E3 fails hand tracking and runtime. These failures are saved in `failures/roboparty_rpo_gates.json`, `failures/unitree_g1_gates.json`, and `failures/e3_v2_gates.json`. Runtime profiling shows the remaining overhead is solver dominated rather than BVH load or metric evaluation: RPO v2 runs in `mixed` Jacobian mode at about `40.58` output frames/s versus legacy `analytic` mode at `193.65`, Unitree G1 v2 runs in `mixed` mode at about `36.10` output frames/s versus legacy `analytic` mode at `175.17`, and synthetic E3 v2 runs in `mixed` mode at about `42.03` output frames/s versus legacy `analytic` mode at `176.17`. Solver-objective diagnostics show RPO v2 still pays autodiff cost from sparse direction plus pole-vector tasks, while Unitree and synthetic E3 use analytic direction tasks but keep autodiff pole-vector tasks. The `v2_pole_analytic` diagnostic confirms why pole activation is not yet safe: RPO improves to about `74.79` output frames/s but hand/foot RMSE worsens to `0.43/0.45`; Unitree G1 reaches pure `analytic` mode with about `225.45` output frames/s and runtime `0.95s`, but hand/foot RMSE worsens to `0.80/0.58`; synthetic E3 v2 reaches about `288.97` output frames/s and improves root residuals, but hand RMSE remains far above legacy at `1.60`. The runtime Jacobian scheduler now selects pure `analytic` mode whenever every active objective supports analytic Jacobians, so future benchmark-safe pole-vector activation will not remain on a mixed/autodiff tape path. The next acceptance work is reach-limited hand tracking and solver-kernel/runtime reduction, especially replacing or damping pole-vector analytic activation so it preserves bounded tracking/root gates.
...
Tangent-space pole diagnostics (`v2_pole_tangent_analytic`) validate a lower-dimensional pole objective that projects the pole-normal error into the target normal's tangent plane. On a warm-cache bounded run it reduces sparse residual dimension from `66` to `58`; Unitree G1 and synthetic E3 v2 reach pure `analytic` mode with solve FPS `141.56` and `218.67` respectively, while RPO remains `mixed` because its direction objective is still autodiff. The mode is still not benchmark-safe as a default: RPO hand/foot RMSE worsens from standard v2 `0.39/0.41` to `0.43/0.45`, Unitree improves hand/foot RMSE to `0.54/0.31` but must still be evaluated against the full default gate set, and E3 hand RMSE improves only from `1.20` to `1.05`, still far above legacy `0.59`. This makes tangent-space pole residuals a useful runtime/tracking diagnostic, not an accepted default schedule.
