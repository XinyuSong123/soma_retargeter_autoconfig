...
- Contact locking supports hysteresis, per-environment weights, minimum stance duration, and release blending for batch-stable foot targets.
...
Compiled profiles include lightweight self-collision proxy metadata when body geoms or chain-length fallbacks are available. Runtime collision barriers are still reported as not implemented in benchmark artifacts.
